...
1. assets から `business-requirements.md` をコピーし、要望に沿って記載
2. `phase.yaml`: `current_phase: 2`, `waiting_approval: true`, `status: in_progress`
3. **通知**: `bash scripts/ntfy.sh "📋 ビジネス要件を書きました。レビューをお願いします"`
4. **⛔ 承認を待つ — ここで会話を終了。次の発言まで何もしない**

### フェーズ 3: system_requirements

1. assets から `system-requirements.md` をコピーし、要望に沿って記載
2. `phase.yaml`: `current_phase: 3`, `waiting_approval: true`, `status: in_progress`
3. **通知**: `bash scripts/ntfy.sh "📋 システム要件を書きました。レビューをお願いします"`
4. **⛔ 承認を待つ — ここで会話を終了。次の発言まで何もしない**
